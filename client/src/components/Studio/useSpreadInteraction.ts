...
import type { BookElement, BookFrame, BookPageSetup, BookSpread } from '../../types/book'
import { useStudioStore } from '../../store/studioStore'

/**
 * Moving and resizing on the sheet. Ported from liketrek/trek's
 * client/src/components/Studio/useSpreadInteraction.ts (same AGPLv3
 * license) — plain pointer-event math, no external drag library, no
 * server/Nest coupling beyond the type import.
 *
 * Hand-rolled on pointer events rather than a drag-and-drop library, because
 * the libraries in this space move items between lists — they have no
 * notion of a free frame, a resize handle, a rotation, or a magnetic edge.
 *
 * Snapping is what separates an editor that feels precise from one that
 * feels slippery. Candidate lines come from the page (edges, centres, safe
 * area, the gutter) and from every other element on the spread (its edges
 * and centres). The closest candidate within the threshold wins, the offset
 * is applied, and the line is drawn so you can see *why* it stopped there.
 */
...
export function snapTargets(spread: BookSpread, page: BookPageSetup, exclude: Set<string>) {
  ...
  // A spread has four vertical safe lines, not two: the renderer draws the
  // safe area per page, including the two either side of the fold.
  if (!single) {
    xs.push(
      page.pageWidth,
  ...
  // The centre of each page, and the quarters within it — on a spread
  // `w / 2` is the fold, not the middle of anything being composed.
...
export function useSpreadInteraction(opts: {
  ...
  const onPointerMove = useCallback((e: React.PointerEvent) => {
    ...
    if (g.kind === 'rotate') {
      ...
      // taking the pointer's own angle would snap the element to wherever
    ...
    if (g.kind === 'move') {
      // Snap on the union of all moving edges, so a multi-selection lines up
      // as a block rather than each element fighting for its own line.
      const movedX: number[] = []
      const movedY: number[] = []
  ...
  const startMove = useCallback((e: React.PointerEvent, el: BookElement) => {
    ...
    // Locked means "cannot be moved", not "cannot be reached" — selection
    // happens first (so the inspector can show it, and the unlock button is
    // reachable) and the gesture stops here.
    if (el.locked) return
  ...
  /**
   * Take hold of the rotation handle. The centre comes from the selection's
   * own box rather than from the handle, so a multi-selection turns as one
   * object around the middle of the group.
   */
