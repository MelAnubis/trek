...
/**
 * Layout templates for a spread. Ported from liketrek/trek's
 * client/src/components/Studio/templates.ts (same AGPLv3 license) — pure
 * layout math, fully portable.
 *
 * A template is not a special mode and not a locked page — it is a set of
 * frames with the pictures left out. Applying one keeps the photos and the
 * words that are already on the spread and pours them into the new
 * arrangement, in order; anything the new layout has room for but the old
 * one did not fill stays an empty frame you can drop a picture onto.
 *
 * Content the new layout has no room for is **parked, not deleted**. Trying
 * a text-only layout and then changing your mind must not cost you the
 * pictures — they come straight back the moment a layout with frames is
 * applied.
 */
...
export interface Template {
  ...
  photoSlots: number
  /** A cover is one page rather than two; everything without this is an inner (spread) layout. */
  role?: 'inner' | 'single'
 ...
 *
 * Order is kept, because order is the only thing that survives a layout
 * change with its meaning intact: the first photo stays the first photo.
 */
export function applyTemplate(spread: BookSpread, tpl: Template, page: BookPageSetup): BookSpread {
  ...
  // Which text is the heading, which the story, which the date line. Size
  // decides the heading first, and length only sorts what is left.
  const heading = [...texts]
  ...
  for (const slot of slots) {
    if (slot.kind === 'photo') {
      ...
      out.push({
        ...
        // The cut and the decoration travel with the picture. A layout
        // change rearranges a page; it is not a reason for a photograph to
        // come out of its Polaroid.
  ...
  // Everything the arrangement could not take. An empty photo frame carries
  // no picture, so it is not worth parking; a photo with an id and a text
  // that was never placed are.
  const parked = [
