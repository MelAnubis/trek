...
function PhotoGrid({ photos, onClick }: { photos: JourneyPhoto[]; onClick: (idx: number) => void }) {
  ...
  // object-contain (not cover) so the full photo always shows — a portrait
  // shot in a landscape cell gets a neutral fill behind it instead of being
  // cropped to the cell's aspect ratio.
  if (count === 1) {
    return (
      <div className="overflow-hidden cursor-pointer" onClick={() => onClick(0)}>
        <PhotoImg photo={photos[0]} className="w-full h-72 object-contain bg-zinc-100 dark:bg-zinc-800" />
      </div>
    )
  }
    ...
    return (
      <div className="grid grid-cols-2 gap-0.5 overflow-hidden">
        {photos.slice(0, 2).map((p, i) => (
          <PhotoImg key={p.id} photo={p} className="w-full h-52 object-contain bg-zinc-100 dark:bg-zinc-800 cursor-pointer" onClick={() => onClick(i)} />
        ))}
      </div>
    )
  }
  ...
  return (
    <div className="overflow-hidden flex" style={{ height: 300, gap: 2 }}>
      <div className="flex-1 min-w-0 cursor-pointer" onClick={() => onClick(0)}>
        <PhotoImg photo={photos[0]} className="w-full h-full object-contain bg-zinc-100 dark:bg-zinc-800" />
      </div>
      <div className="flex-1 min-w-0 flex flex-col" style={{ gap: 2 }}>
        <div className="flex-1 min-h-0 cursor-pointer" onClick={() => onClick(1)}>
          <PhotoImg photo={photos[1]} className="w-full h-full object-contain bg-zinc-100 dark:bg-zinc-800" />
        </div>
        <div className="flex-1 min-h-0 relative cursor-pointer" onClick={() => onClick(2)}>
          <PhotoImg photo={photos[2]} className="w-full h-full object-contain bg-zinc-100 dark:bg-zinc-800" />
          {count > 3 && (
