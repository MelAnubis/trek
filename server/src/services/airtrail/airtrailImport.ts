...
export interface AirtrailImportResult {
  imported: string[];
  skipped: Array<{
    flightId: string;
...
export async function importAirtrailFlights(
  ...
  for (const flight of selected) {
    ...
    try {
      ...
      (reservation as any).external_source = 'airtrail';
      (reservation as any).external_id = fid;
      (reservation as any).external_owner_user_id = userId;
      (reservation as any).sync_enabled = 1;
      (reservation as any).external_synced_at = now;
