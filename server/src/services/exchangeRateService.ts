 ...
 *
 * Fetches from exchangerate-api.com (no key) and caches per base currency
 * in-memory for 6 hours. rates[X] = units of X per 1 base, so an amount in
 * currency C converts to base as `amount / rates[C]`.
 *
 * Everything degrades gracefully: if the fetch fails (offline, upstream down),
 * callers get null and amounts are treated as already in the base currency.
 */
