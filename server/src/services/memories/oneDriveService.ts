...
export async function listAlbums(userId: number) {
  ...
  // Fallback / complement: walk the Photos folder tree (same bounded walk
  // as searchPhotos()) and treat any folder with a name a person would
  // actually choose — not a bare year or month — as an album. Filters out
  // Camera Roll's auto date folders while still catching manually-named
  // trip folders, without requiring the Album feature to work at all.
  ...
  while (queue.length > 0 && foldersVisited < MAX_FOLDERS) {
    ...
    for (const item of result.data?.value || []) {
      ...
      if (looksAutoGenerated(item.name)) continue;
  ...
  console.log(`[oneDrive] listAlbums: ${bundleAlbumCount} real album(s) + ${albums.length - bundleAlbumCount} named folder(s), ${foldersVisited} folder(s) visited`);
  return { albums };
}
