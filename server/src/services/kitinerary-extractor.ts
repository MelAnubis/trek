...
let binaryPath: string | null | undefined = undefined; // undefined = not yet resolved
...
function getBinaryPath(): string | null {
  if (binaryPath === undefined) {
    ...
    if (binaryPath) {
      console.log(`[KItinerary] extractor found at: ${binaryPath}`);
    } else {
...
export function isKitineraryAvailable(): boolean {
  return getBinaryPath() !== null;
}

export async function extractBooking(buffer: Buffer, fileName: string): Promise<KiReservation[]> {
  ...
  try {
    ...
    const { stdout, stderr } = await execFileAsync(path, [tmpFile], {
      timeout: TIMEOUT_MS,
